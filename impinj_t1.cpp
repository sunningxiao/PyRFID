 ...
 *记录修改内容
 *1.删除main函数
 *2.修改CMyApplication中的run，目前run函数没有作用
 *3.增加connnect函数，用于连接impinj,输入ip地址，在调用connect函数前对m_Verbose赋值，初始值为0，最大值为2
 *4.增加close函数，用于关闭impinj
 *5.
 *
 *
 *
...
int CMyApplication::connect(char * pReaderHostName)
{	
...
//    CConnection *               pConn;
     ...
     */
    m_pConnectionToReader = new CConnection(pTypeRegistry, 32u*1024u);
 ...
 */
    ...
    for(
        CustCur = pRO_ACCESS_REPORT->beginCustom();
        CustCur != pRO_ACCESS_REPORT->endCustom();
        CustCur++)
    {
        if((*CustCur)->m_pType == &CImpinjTagInformation::s_typeDescriptor)
        {
            printImpinjTagInformation((CImpinjTagInformation *) *CustCur);
        }
    }
 ...
 *****************************************************************************
 **
 ** @brief  Helper routine to print a Impinj Tag Data
 **
 **
 ** @return     void
 **
...
void
CMyApplication::printImpinjTagInformation (
  CImpinjTagInformation *           pImpinjTagInfo)
{
    char aBuf[64];
    CEPCData *pepc;
    CImpinjTagDirection *pImpTagDir;

    pepc = pImpinjTagInfo->getEPCData();
    pImpTagDir = pImpinjTagInfo->getImpinjTagDirection();

    formatOneEPC(pepc, aBuf, 256, "epc=");

    if(NULL != pImpTagDir)
    {
        llrp_u16_t confidence = pImpTagDir->getConfidence();
        EImpinjDirection direction = pImpTagDir->getTagDirection();
        printf("INFO: DIRECTION %s Direction %d, Confidence %u\n", 
            aBuf, direction, confidence);
    }
}
...
CMyApplication::printOneTagReportData (
  CTagReportData *              pTagReportData, char *ptr)
{
    char                        epcBuf[128];
   // char                        aBuf[128];
   // char                        *ptr = aBuf;
